...
namespace jpeg_decoder {
    ...
    struct YCbCr {
        ...
        explicit operator jpeg_decoder::RGB() const {
            const uint8_t r = Clamp(y + ((91881 * (cr - 128)) >> 16));
            const uint8_t g = Clamp(y - ((22554 * (cb - 128) + 46802 * (cr - 128)) >> 16));
            const uint8_t b = Clamp(y + ((116130 * (cb - 128)) >> 16));
            return {r, g, b};
        }
    };
}
